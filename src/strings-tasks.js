...
function isPalindrome(str) {
  let clearedStr = str.toLowerCase();
...
function invertCase(str) {
  ...
  return str
    .split('')
    .map((letter) => invertLetter(letter))
    .join('');
}
...
function extractNameFromTemplate(value) {
  let firstClean = removeFirstOccurrences(value, 'Hello, ');
  firstClean = removeFirstOccurrences(firstClean, '!');
  return firstClean;
}
...
function encodeToRot13(str) {
  const given = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const ROT13 = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  const encode = (letter) => ROT13.charAt(given.indexOf(letter));
  return str
    .split('')
    .map((letter) => {
      const encodedLetter = encode(letter);
      if (encodedLetter) return encodedLetter;
      return letter;
    })
    .join('');
}
...
function getCardId(value) {
  ...
  return deck.findIndex((curr) => curr === value);
}
